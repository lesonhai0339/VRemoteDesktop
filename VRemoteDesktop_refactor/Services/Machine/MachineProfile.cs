...
namespace Vsign4.VRemoteDesktop.Services.Machine
{
    ...
    public class MachineProfile : IMachineProfile, IDisposable
    {
        ...
        {
            string filePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), HeaderSchema.RootFolder, HeaderSchema.PasswordFile);
        ...
        {
            // Runs during construction (InitializeMachineInfo). A corrupt password file must
            // not throw here or the whole app fails to start.
            try
            {
                string filePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), HeaderSchema.RootFolder, HeaderSchema.PasswordFile);
            ...
            {
                Console.WriteLine("GetDefaultPassword error: " + ex.Message);
            }
            return "";
        }
        ...
        {
            // Runs during construction. When the machine is offline, socket.Connect throws a
            // SocketException that would otherwise crash app start-up. Fall back gracefully.
            try
            {
            ...
            {
                Console.WriteLine("LocalIpAddress error, falling back: " + ex.Message);
            }
            ...
            {
                Console.WriteLine("LocalIpAddress DNS fallback error: " + ex.Message);
            }
