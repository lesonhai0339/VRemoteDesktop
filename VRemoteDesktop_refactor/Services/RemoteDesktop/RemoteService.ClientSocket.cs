    ...
    public partial class RemoteService
    {
        ...
        public ClientSession NewControlled(string sessionId, int width, int height)
        {
            if (string.IsNullOrWhiteSpace(sessionId))
                sessionId = StringHelper.RandomStringNumber(HeaderSchema.SessionIdLength);
        ...
        private void GetPartnerInfoSuccessCallback(byte[] data)
        {
            ...
            bool isSuccess = clientSession.TryConnect(connectIP, int.Parse(DEFAULT_REMOTE_PORT), HeaderSchema.RetryAttemp, HeaderSchema.TimeoutMilisecond);
        ...
        private void CreateRemoteConnection(object sender, byte[] data)
        {
            ...
            bool isSuccess = clientSession.Listen(int.Parse(DEFAULT_REMOTE_PORT), HeaderSchema.TimeoutMilisecond);

            if (!isSuccess)
            {
                bool connectSuccess = clientSession.TryConnect(DEFAULT_SERVER_IP, int.Parse(DEFAULT_REMOTE_PORT), HeaderSchema.RetryAttemp, HeaderSchema.TimeoutMilisecond);
                if (!connectSuccess)
                {
                    if(OnError != null)
                ...
                clientSession.AddWork(priority: QueuePriority.High, task: new TaskObject(SocketDataType.RemoteLogin, packet));
            }
        }
        private void EstablishRelayConnect(ClientSession clientSession, PartnerNetworkInfo partnerInfo)
        {
            bool isSuccess = clientSession.TryConnect(DEFAULT_SERVER_IP, int.Parse(DEFAULT_REMOTE_PORT), HeaderSchema.RetryAttemp, HeaderSchema.TimeoutMilisecond);
            if (!isSuccess)
            {
                if(OnError != null)
        ...
        private void RemoteLoginCallback(object sender, byte[] data)
        {
            try
            {
                ...
                if (clientSession == null)
                {
                    // Was dereferencing the null session (guaranteed NullReferenceException).
                    // Nothing to notify on a null session; just bail out.
                    Logger.Log.ForContext("FileName", GetType().Name).Error("RemoteLoginCallback received a null client session");
                    return;
                }
                ...
                {
                    clientSession.AddWork(priority: QueuePriority.High, task: new TaskObject(SocketDataType.RemoteLoginFailed));
                ...
                var myInfo = new RemoteLoginResponse(loggedIn: true, connectionId: clientSession.SessionId, machineInfo: _machineProfile.MachineInfo);
                ...
                clientSession.AddWork(priority: QueuePriority.High, task: new TaskObject(SocketDataType.RemoteLoginSuccess, packet));
        ...
        public void SaveConnectHistory(IEnumerable<ConnectHistoryInfo> items)
        {
            foreach (var item in items)
            {
                SaveConnectHistory(item);
            }
        }
        public void SaveConnectHistory(ConnectHistoryInfo historyInfo)
        {
            try
            {
                string historyFilePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), HeaderSchema.RootFolder, HeaderSchema.ConnectionHistoryFile);

                string data = $"{historyInfo.Id}{SEP}{historyInfo.Name}{SEP}{historyInfo.LastConnect}";
                var encryptedData = VCrypto.EncryptString(data);

                FileHelper.WriteToFile(data: encryptedData, filePath: historyFilePath, append: true);
            }
            catch(Exception ex)
            {
                Logger.Log.ForContext("FileName", GetType().Name).Error(ex, "Save connection history error");
            }
        }
        public ConnectHistoryInfo[] LoadConnectHistory()
        {
            try
            {
                string historyFilePath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), HeaderSchema.RootFolder, HeaderSchema.ConnectionHistoryFile);

                var encryptedHistory = FileHelper.ReadFromFile2(historyFilePath);

                if (encryptedHistory == null || encryptedHistory.Count() == 0)
                    return new ConnectHistoryInfo[0];

                var listHistory = VCrypto.DecryptString(encryptedHistory.ToArray());

                var data = listHistory.Select(x =>
                {
                    var strings = x.Split(SEP);
                    ConnectHistoryInfo info = new ConnectHistoryInfo
                    {
                        Id = strings[0],
                        Name = strings[1],
                        LastConnect = strings[2]
                    };
                    return info;
                }).ToArray();
            ...
            {
                Logger.Log.ForContext("FileName", GetType().Name).Error(ex, "Load connection history error");
                return new ConnectHistoryInfo[0];
            }
        }
        #endregion
