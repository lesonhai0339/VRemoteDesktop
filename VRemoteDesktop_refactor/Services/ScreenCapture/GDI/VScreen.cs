...
namespace Vsign4.VRemoteDesktop.Services.ScreenCapture.GDI
{
    public abstract class VScreen : IDisposable
    {
        ...
        /// <returns></returns>
        public virtual Interop.CaptureApi.BITMAPINFO InitBitmapInfo(
            int width,
            int height,
            ushort bitPerPixel,
            uint compression = 0)
        {
            var bmi = new Interop.CaptureApi.BITMAPINFO();
            bmi.Header.biSize = (uint)Marshal.SizeOf(typeof(Interop.CaptureApi.BITMAPINFOHEADER));
        ...
        protected virtual void InitCaptureBuffer(
            ...
            uint offset,
            IntPtr screenDCPtr,
            Interop.CaptureApi.BITMAPINFO bmi,
            ...
            int bytePerPixel)
        {
            ...
            *d++ = (uint)regionHeight;
            ...
            uint dstStride = (uint)(regionWidth * bytePerPixel); // unpadded, matches MergeRegionToSource expectation
        ...
        {
            // Order matters: reject negative width/height FIRST so the following subtractions
            // cannot be exploited. The original "x + width > dstWidth" could integer-overflow
            // with a malformed (huge) width from the peer, bypass the check, and drive an
            // out-of-bounds memcpy. Comparing against (dstWidth - width) avoids the overflow.
            if (width <= 0 || height <= 0 || x < 0 || y < 0 || bytePerPixel <= 0)
                return 0;
            if (x > dstWidth - width || y > dstHeight - height)
                return 0;
