...
namespace Vsign4.VRemoteDesktop.Services.ScreenCapture.GDI
{
    ...
    public class VScreenSender : VScreen, IVScreenSender, IDisposable
    {
        ...
        private Interop.CaptureApi.BITMAPINFO _bitmapInfo;
        ...
        public void RemoveSessionBuffer(string id)
        {
            lock (_lock)
            {
                VBufferSwapper swapper; 
                _clientSessionBufferSwapper.TryRemove(id, out swapper);
            }
        }
        /// <summary>
        ...
        {
            double ticksPerFrame = (double)Stopwatch.Frequency / _limitFramePerSecond;
            ...
            while (!token.IsCancellationRequested)
            {
                ...
                {
                    long elapsedTicks = st.ElapsedTicks;
                    long remainingTicks = (long)ticksPerFrame - elapsedTicks;

                    if (remainingTicks > 0)
                    {
                        double wTime = (double)remainingTicks * 1000 / Stopwatch.Frequency;
