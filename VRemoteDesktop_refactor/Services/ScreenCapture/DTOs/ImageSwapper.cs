        ...
        public void Add(RegionFrame regions)
        {
            ...
            lock (_lock)
            {
                ...
                for (int r = start; r < length; r++)
                {
                    ...
                    var region = regions.Bounds[r]; 
                    //luôn chia hết cho 16 nên có thể tính trực tiếp
                    int startCol = region.X;
                    int startRow = region.Y;

                    int numCols = (region.Width + _size - 1) >> _shift;
                    int numRows = (region.Height + _size - 1) >> _shift;
                    //int numCols = (region.Width + 31) >> 4;
                    //int numRows = (region.Height + 31) >> 4;
